# ============================================================================
# Phase 6 — 금융 도메인 어댑터 LoRA 파인튜닝 (Colab Pro T4용)
#
# 분리형 아키텍처의 1번째 어댑터: "동음이의어를 문맥으로 구별하는 능력"만
# 가르친다(언어 무관, 핵심 IP). 언어별 유창성 보정(언어 어댑터)은 별도 단계.
#
# 대조 손실은 인코더(언어 무관 공유 부분)의 한국어 원문 표현에만 건다 --
# "같은 용어라도 금융 문맥이면 A, 비금융 문맥이면 B로 다르게 표현하라"를
# 인코더에 새기는 것이 정확히 이 아키텍처가 원하는 지점이다(디코더/타겟언어
# 쪽에 걸면 언어별 특성과 섞여버린다).
#
# 아래는 "# ===== Cell N =====" 구분선 기준으로 Colab 코드 셀 하나씩에
# 그대로 복사해 넣으면 된다. 각 셀은 이전 셀에서 만든 변수를 그대로 쓴다
# (한 노트북 안에서 순서대로 실행하는 걸 전제로 함 -- 표준 Jupyter/Colab 동작).
#
# 데이터 전제: contrastive_train.csv / contrastive_val.csv / contrastive_test.csv
# 를 Google Drive에 먼저 업로드해둘 것 (컬럼: term, grade, sense, ko_sentence,
# lang, translation, label, provider, labse_filter_pass).
# ============================================================================
...
#
# !pip install -q transformers peft accelerate sentencepiece pandas sacremoses
...
DRIVE_ROOT = "/content/drive/MyDrive/finholly"  # 이 아래에 데이터/결과 저장
...
# 위 3개 CSV를 미리 DATA_DIR에 업로드해두세요
# (로컬 프로젝트의 contrastive_train.csv / contrastive_val.csv / contrastive_test.csv 그대로)
...
# term 분리 검증 -- test/val 용어가 train에 하나도 없어야 함(일반화 시험 전제)
...
# LoRA 설정 -- T4(15GB) 보수적으로: rank 낮게, attention 프로젝션만
lora_config = LoraConfig(
    task_type=TaskType.SEQ_2_SEQ_LM,
    r=16,
    lora_alpha=32,
    lora_dropout=0.05,
    target_modules=["q_proj", "v_proj"],  # 여유 있으면 ["q_proj","k_proj","v_proj","out_proj"]로 확장
    bias="none",
)
model = get_peft_model(model, lora_config)
model.print_trainable_parameters()  # 전체 대비 학습 파라미터 비율 확인 (LoRA면 보통 1% 미만)

device = "cuda" if torch.cuda.is_available() else "cpu"
model.to(device)


# ===== Cell 5: Dataset / 용어단위 배치 샘플러 / collate_fn =====
...
def make_collate_fn(tokenizer, src_lang=SRC_LANG, max_length=MAX_LENGTH):
    def collate(batch):
        ...
        # 타겟 언어가 행마다 다르므로 언어별로 그룹 지어 따로 인코딩한 뒤 원래 순서로 재배치
        ...
        for lang, idxs in by_lang.items():
            ...
            # as_target_tokenizer()는 최신 transformers에서 deprecated ->
            # text_target= 방식으로 교체 (tgt_lang 속성은 그대로 참조됨)
...
TERMS_PER_BATCH = 2
SAMPLES_PER_TERM = 4  # 배치 크기 = TERMS_PER_BATCH x SAMPLES_PER_TERM = 8 (T4 보수적)
GRAD_ACCUM_STEPS = 1  # 실효 배치 = 32
...
# ===== Cell 6: 대조 손실 (인코더 표현 기반, TermGPT식 동음이의어 구별) =====
...
# ===== Cell 7: 학습 루프 (체크포인트 저장 포함) =====

from transformers import get_linear_schedule_with_warmup

EPOCHS = 30  # 첫 실행은 끝까지 에러 없이 도는지 확인용. 성공하면 12로 올릴 것
LR = 5e-4
CONTRASTIVE_WEIGHT = 0.3  # CE loss 대비 대조 loss 비중
WARMUP_RATIO = 0.03
...
scaler = torch.cuda.amp.GradScaler()  # T4는 bf16 미지원이라 fp16 + GradScaler
...
for epoch in range(EPOCHS):
    ...
    # 에폭마다 체크포인트 저장 (Colab 세션 끊겨도 Drive에 남음)
...
# ===== Cell 8: 평가 함수 (LaBSE 기반 — 순정 vs 어댑터 비교) =====

# LaBSE로 "생성된 번역이 의도된 sense(금융/비금융)에 더 가까운지" 측정.
# 방법론은 로컬 프로젝트의 Phase 4-A 승인기준(LaBSE 역번역 검증)과 동일한
# 원리 -- 생성문이 정답(gold) 쪽에 더 가깝고 반대 sense 쪽보다 멀면 "정확".
...
# 다음 단계 메모:
# 1) 언어 어댑터(언어별 유창성)는 이 도메인 어댑터 위에 별도 LoRA로 쌓거나,
#    merge_and_unload() 후 재학습 -- 별도 데이터/스크립트 필요.
# 2) 로컬 온프레미스 배포(CTranslate2 INT8)로 옮기려면:
#    merged = model.merge_and_unload() 후 HF 포맷으로 저장 ->
#    ct2-transformers-converter로 CTranslate2 변환 (기존 파이프라인과 동일 포맷).
# 3) val 세트(차주/간주/인수)로 하이퍼파라미터(LR, CONTRASTIVE_WEIGHT, EPOCHS)
#    튜닝 여지 있음 -- 지금은 T4 보수적 기본값.
