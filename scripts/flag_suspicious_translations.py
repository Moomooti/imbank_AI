...
WORK_CSV = ROOT / "data" / "qa_review" / "suspicious_work.csv"  # 역번역 원시결과 누적(이어하기용)
...
def back_translate_all(df: pd.DataFrame, limit: int | None) -> None:
    ...
    for lang, prefix in LANG_PREFIX.items():
        ...
        if lang == "tgl_Latn":
            candidates = candidates[candidates[src_col] != "glossary"]  # loanword 통과는 역번역 대상 제외
...
def build_report() -> pd.DataFrame:
    ...
    # ko_term은 다의어(ANNA/ISIN/CDS 등 senses)나 완전중복행 때문에 유일 키가 아닐 수 있음.
    # is_short/is_dup은 표제어 단위 속성이라 중복행 간에도 값이 같으므로 첫 값만 취해 dedupe.
