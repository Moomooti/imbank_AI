...
# 팀원 원본 파일은 Downloads에 있고, macOS발 NFD(분해형) 유니코드 파일명이라
# Windows에서 문자열 리터럴로 직접 못 연다 -> NFC 정규화로 찾아서 프로젝트
# 안에 정상 이름 사본을 한 번만 만들어둔다.
...
# Task A와 동일한 프로덕션 백엔드 조합 (왕복검증 때와 달리 언어당 백엔드 1개만)
LANG_TARGETS = {
    "vie_Latn": ("deepl", "VI"),
    "ind_Latn": ("deepl", "ID"),
    "tha_Thai": ("deepl", "TH"),
...
def main():
    # Windows 콘솔 기본 코드페이지(cp949)는 태국어/미얀마어를 못 찍어서 마지막
    # 디버그 출력에서 UnicodeEncodeError로 죽는 문제가 있었음 -> utf-8로 강제.
    try:
    ...
    for lang, (backend, deepl_lang) in LANG_TARGETS.items():
        ...
        # confidence-gated 언어(필리핀/미얀마)는 allowlist에 없는 용어를 애초에
        # 검색 대상에서 뺀다 -> 그 문장은 masking 없이 원문 그대로 순정 NLLB로
        # 번역됨. 언어마다 검색 대상이 달라지므로 mask는 언어별로 다시 수행.
        masked_records = []
        for row in src_rows:
