...
from app.terms import GlossaryRow, apply_matches, build_rows, find_term_matches, load_merged  # noqa: E402
from app.terms.matcher import TermMatch, _resolve_overlaps  # noqa: E402
...
def run_case(label: str, lang: str, text: str, rows: list[GlossaryRow], expect_terms: set[str]) -> str:
    ...
    # 매치 구간 바깥은 원문과 한 글자도 달라지면 안 된다(오프셋 안 밀림 검증).
...
def check_overlap_resolution() -> None:
    ...
    떼어 검증한다."""
    candidates = [
        TermMatch(10, 20, "x" * 10, "짧은용어A", match_priority=1, is_loanword=False),  # 길이 10, priority 1
        TermMatch(10, 25, "x" * 15, "긴용어B", match_priority=999, is_loanword=False),  # 길이 15 (더 김) -> 이겨야 함
        TermMatch(12, 18, "x" * 6, "겹치는짧은용어C", match_priority=0, is_loanword=False),  # B와 겹침 -> 탈락해야 함
        TermMatch(30, 40, "y" * 10, "안겹치는용어D", match_priority=5, is_loanword=False),  # 완전히 별개 구간
    ]
...
def main():
    ...
    # --- 태국어 (공백 없는 문자, Phase 2 분절기 + custom_dict) ---
    # 일부러 용어 앞뒤에 공백 없이 바로 붙인, 실제 MT 출력에서 벌어질 법한
    # 어려운 케이스로 검증한다 (custom_dict 없으면 실패했던 케이스, 앞서 확인함).
    ...
    # --- 미얀마어 (공백 없는 문자, Phase 2 분절기, 자연스러운 절 구분 부호 사용) ---
