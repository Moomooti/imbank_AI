...
class TermMatch:
    ...
    matched_text: str  # text[start:end] (디버깅/검증용)
    ko_term: str
    match_priority: int
    is_loanword: bool
...
def _find_tokenized(text: str, lang: str, rows: list[GlossaryRow]) -> list[TermMatch]:
    ...
    if lang == "tha_Thai":
        # 실측 확인된 문제: 커스텀 사전 없이는 용어가 공백 없이 다른 말과 바로
        # 붙으면 newmm이 용어 시작 지점을 옆 단어와 뭉쳐버릴 때가 있다(예:
        # "...ใช้งาน"+"การหักบัญชี..." -> "งานการ"로 잘못 뭉침). 이번에 매칭할
        # 후보 용어들을 Trie로 넘겨서 newmm이 그 경계를 우선 잡게 한다.
        # 주의: 텍스트뿐 아니라 용어 자체를 토큰화할 때도 반드시 "같은" custom_dict를
        # 써야 한다 — 안 그러면 커스텀 사전 적용 시 텍스트 쪽은 용어가 토큰 1개로
        # 뭉치는데 용어 쪽은 기본 newmm으로 여러 토큰으로 쪼개져서 길이가 안 맞아
        # 매칭이 전부 실패한다(실제로 이 버그로 한 번 실패한 뒤 고침).
