...
MASK_TEMPLATE = "{{{{FINTERM{:03d}}}}}"  # -> "{{FINTERM001}}"
...
class MaskEntry:
    ...
    ko_term: str
    match_priority: int
    order: int  # 1-based, 원문에서 등장한 순서(왼쪽부터)
...
class RestoreReport:
    total: int = 0
    survived: int = 0  # 식별자 문자열이 번역 결과에 그대로 남아있었던 개수
    order_preserved_pairs: int = 0  # 살아남은 식별자들끼리 순서가 유지된 인접쌍 수
    order_total_pairs: int = 0
    restored: int = 0  # 실제로 목표언어 용어로 복원된 개수
    fallback_used: int = 0  # 유실/번역누락으로 폴백 처리된 개수
...
def restore_text(translated: str, entries: list[MaskEntry], lang_rows: list[GlossaryRow]) -> tuple[str, RestoreReport]:
    ...
    restored = 0
    fallback_used = 0
    for idx, e in sorted(found, key=lambda x: x[0], reverse=True):  # 뒤에서 앞으로
        restore_value = term_by_ko.get(e.ko_term, "")
        if restore_value.strip():
            value, status = restore_value, "restored"
            restored += 1
        else:
            value, status = e.ko_term, "fallback_no_translation"  # 그 언어 번역이 없으면 한국어 원어로 폴백
            fallback_used += 1
